...
public class HomeDog implements DogGameInterface{
	...
	public int lunchChoice() {
		...
		while(true) {
			...
			if (rice <= 100 && 0 <= rice) {
				if (50 < rice) {
					System.out.println("다이어트 해야해..");
					heartDegree.heartDegree(-10);
				} else {
					System.out.println("속이 편해서 좋아..");
					heartDegree.heartDegree(20);
				}
				break;
			} else {
		...
		while(true) {
			...
			if (water <= 100 && 0 <= water) {
				if (50 < water) {
					System.out.println("오줌.. 귀찮아..");
					heartDegree.heartDegree(-10);
				} else {
					System.out.println("어처피 목 안말라..");
					heartDegree.heartDegree(20);
				}
				break;
			} else {
