...
public class GangDog implements DogGameInterface{
	...
	public int lunchChoice() {
		...
		while(true) {
			...
			if (rice <= 100 && 0 <= rice) {
				if (50 < rice) {
					System.out.println("단백질 줘");
					heartDegree.heartDegree(-10);
				} else {
					System.out.println("저탄고지 좋아");
					heartDegree.heartDegree(20);
				}
				break;
			} else {
		...
		while(true) {
			...
			if (water <= 100 && 0 <= water) {
				if (50 < water) {
					System.out.println("수분은 섭취해야지");
					heartDegree.heartDegree(20);
				} else {
					System.out.println("물은 내놔야지");
					heartDegree.heartDegree(-10);
				}
				break;
			} else {
