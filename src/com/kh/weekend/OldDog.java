...
public class OldDog implements DogGameInterface{
	...
	public int suddenEventChance() {
		...
		for (int i = 0; i < 1; i++) {		
			int eventResult = rand.nextInt(7)+1;
			if(eventResult == 1) {
				suddenCat();
	...
	public int lunchChoice() {
		...
		while(true) {
			...
			if (rice <= 100 && 0 <= rice) {
				if (50 < rice) {
					System.out.println("속이 더 부룩해..");
					heartDegree.heartDegree(-10);
				} else {
					System.out.println("깔끔해서 좋구먼..");
					heartDegree.heartDegree(20);
				}
				break;
			} else {
		...
		while(true) {
			...
			if (water <= 100 && 0 <= water) {
				if (50 < water) {
					System.out.println("수분은 중요하지..");
					heartDegree.heartDegree(20);
				} else {
					System.out.println("컥컥");
					heartDegree.heartDegree(-10);
				}
				break;
			} else {
		...
		while(true) {
			...
			if (snack <= 100 && 0 <= snack) {
				if (50 < snack) {
					System.out.println("나 당뇨 걸린다..");
					heartDegree.heartDegree(-10);
				} else {
					System.out.println("건강이 최고여~");
					heartDegree.heartDegree(20);
				}
				break;
			} else {
